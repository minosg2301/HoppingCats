    ...
    public static class SaveGame
    {
        ...
        public static void Save<T>(string identifier, T obj, bool encode, string password, ISaveGameSerializer serializer, ISaveGameEncoder encoder, Encoding encoding, SaveGamePath path)
        {
            ...
            if (encode)
            {
                ...
                {
                    SaveGameWebGL.SaveData(filePath, encoded);
                }
                else
                {
                    if (!usePlayerPrefs)
            ...
            else if (usePlayerPrefs)
            {
                ...
                if (Application.platform == RuntimePlatform.WebGLPlayer)
                {
                    SaveGameWebGL.SaveData(filePath, data);
                }
                else
                {
        ...
        public static T Load<T>(string identifier, T defaultValue, bool encode, string password, ISaveGameSerializer serializer, ISaveGameEncoder encoder, Encoding encoding, SaveGamePath path)
        {
            ...
            if (encode)
            {
                ...
                if (!usePlayerPrefs)
                {
                    if (Application.platform == RuntimePlatform.WebGLPlayer)
                    {
                        data = SaveGameWebGL.ReadData(filePath);
                    }
                    else
                    {
                ...
                // baontp - keep compat old version
                byte[] bytes;
                try { bytes = System.Convert.FromBase64String(decoded); }
                catch { bytes = encoding.GetBytes(decoded); }
                // baontp - keep compat old version - end
        ...
        public static bool Exists(string identifier, SaveGamePath path)
        {
            ...
            if (!usePlayerPrefs)
            {
                if (Application.platform == RuntimePlatform.WebGLPlayer)
                {
                    return SaveGameWebGL.Exist(filePath);
                }
                else
                {
        ...
        {
            bool result = false;
#if !UNITY_SAMSUNGTV && !UNITY_TVOS && !UNITY_WEBGL || UNITY_EDITOR
