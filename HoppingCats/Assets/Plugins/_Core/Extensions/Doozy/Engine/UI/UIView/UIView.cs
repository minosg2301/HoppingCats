...
//                    yield return new WaitForSecondsRealtime(HideBehavior.Animation.TotalDuration + UIViewSettings.DISABLE_WHEN_HIDDEN_TIME_BUFFER); //wait for seconds realtime (ignore Unity's Time.Timescale)
        ...
        private static void ExecuteShow(string viewCategory, string viewName, bool instantAction)
        {
            ...
            // baontp - ensure view in database
            EnsureView(viewName);
            // baontp - end
        ...
        /// <summary>
        /// If View does not exists in Database, hot load it from Resources
        /// </summary>
        /// <param name="viewName"></param>
        static void EnsureView(string viewName)
        {
            var view = Database.Find(v => v.ViewName == viewName);
            if(!view)
            {
                var prefab = Resources.Load<UIView>(Path.Combine("UI","Views", string.Format("View - {0}", viewName)));
                if(prefab)
                {
                    var masterCanvas = UICanvas.MasterCanvas;
                    view = Instantiate(prefab, masterCanvas.container ? masterCanvas.container : masterCanvas.transform);
                    view.InstantHide();
                }
            }
        }
