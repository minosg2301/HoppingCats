...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        public bool isAutoExpandLayout;
        ...
        public float ForceScrollPosition
        {
            get
            {
                return _scrollPosition;
            ...
            set
            {
                ...
                if (scrollDirection == ScrollDirectionEnum.Vertical)
                {
                    // set the vertical position
                    if(reverse)
                        _scrollRect.verticalNormalizedPosition = (_scrollPosition / ScrollSize);
                    else
                    _scrollRect.verticalNormalizedPosition = 1f - (_scrollPosition / ScrollSize);
                }
                else
        ...
        public EnhancedScrollerCellView GetCellView(EnhancedScrollerCellView cellPrefab)
        {
            ...
            if(cellView == null)
            {
                ...
                var go = Instantiate(cellPrefab.gameObject, _container, false);
                cellView = go.GetComponent<EnhancedScrollerCellView>();
                cellView.PrefabInstanceID = cellPrefab.GetInstanceID();
        ...
        public void ReloadData(float scrollPositionFactor = 0)
        {
            ...
            {
                // set the vertical position
                _scrollRect.verticalNormalizedPosition = reverse ? 0f : 1f - scrollPositionFactor;
            }
            else
            {
                // set the horizontal position
                _scrollRect.horizontalNormalizedPosition = reverse ? 1f - scrollPositionFactor : scrollPositionFactor;
            }
            ...
            )
        {
            ...
            // cache number of cells
            int numberOfCells = NumberOfCells;

            // convert dataIndex if any
            int originIndex = dataIndex;
            if (reverse)
            {
                dataIndex = numberOfCells - dataIndex - 1;
            }
            ...
            if (cellOffset != 0)
            {
                ...
                // get the cell's size
                var cellSize = _cellViewSizeArray[dataIndex];
            ...
            else
            {
                ...
                float maxPosition = reverse
                    ? ScrollSize
                    : GetScrollPositionForCellViewIndex(_cellViewSizeArray.Count - 1, CellViewPositionEnum.Before);
                //float maxPosition = GetScrollPositionForCellViewIndex(_cellViewSizeArray.Count - 1, CellViewPositionEnum.Before);

                // clamp the scroll position to a valid location
                newScrollPosition = Mathf.Clamp(newScrollPosition - (useSpacing ? spacing : 0), 0, maxPosition);
            }
        ...
        private float _AddCellViewSizes()
        {
            ...
            // cached number of cell for optimizing
            int numberOfCells = NumberOfCells;
        ...
        private EnhancedScrollerCellView _GetRecycledCellView(EnhancedScrollerCellView cellPrefab)
        {
            for(var i = 0; i < _recycledCellViews.Count; i++)
            {
                if(_recycledCellViews[i].PrefabInstanceID == cellPrefab.GetInstanceID())
                {
        ...
        private void _AddCellView(int cellIndex, ListPositionEnum listPosition)
        {
            ...
            if(reverse) dataIndex = numberOfCells - dataIndex - 1;
