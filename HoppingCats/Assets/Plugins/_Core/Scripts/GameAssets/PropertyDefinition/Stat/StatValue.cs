...
namespace moonNest
{
    ...
    public struct StatValue : IEquatable<StatValue>
    {
        ...
        /// <summary>
        /// Explicitly casts this <see cref="StatValue"/> into a double, doing
        /// the necessary conversion if necessary, but throwing an
        /// <see cref="InvalidCastException"/> if not possible.
        /// </summary>
        ...
        public double AsDouble
        {
            get
            {
                switch(m_Type)
                {
                    ...
                    default:
                        throw new InvalidCastException
                            ($"Cannot cast this {nameof(StatValue)} instance into a double, because it contains a {m_Type} value.");
                }
            }
        }
    }
