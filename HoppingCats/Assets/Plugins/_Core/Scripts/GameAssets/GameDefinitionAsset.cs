...
namespace moonNest
{
    public class GameDefinitionAsset : SingletonScriptObject<GameDefinitionAsset>
    {
        ...
        void Init()
        {
            ...
            if (actions.Count == 0)
            {
                actions.Add(new ActionDefinition("Login Day") { id = ActionDefinition.kLoginDay, deletable = false });

                ActionDefinition useCurrency = new ActionDefinition("Use Currency") { id = ActionDefinition.kUseCurrency, deletable = false };
                useCurrency.paramTypes[0] = ActionParamType.Currency;
                actions.Add(useCurrency);

                ActionDefinition openChest = new ActionDefinition("Open Chest") { id = ActionDefinition.kOpenChest, deletable = false };
                openChest.paramTypes[0] = ActionParamType.Chest;
                actions.Add(openChest);

                ActionDefinition completeQuestInGroup = new ActionDefinition("Complete Quest In Group") { id = ActionDefinition.kCompleteQuestInGroup, deletable = false };
                completeQuestInGroup.paramTypes[0] = ActionParamType.QuestGroup;
                actions.Add(completeQuestInGroup);

                ActionDefinition completeTutorial = new ActionDefinition("Complete Tutorial") { id = ActionDefinition.kCompleteTutorial, deletable = false };
                completeTutorial.paramTypes[0] = ActionParamType.Tutorial;
                actions.Add(completeTutorial);
            }

            if (actions.Find(a => a.id == ActionDefinition.kGainCurrency) == null)
            {
                ActionDefinition gainCurrency = new ActionDefinition("Gain Currency") { id = ActionDefinition.kGainCurrency, deletable = false };
                gainCurrency.paramTypes[0] = ActionParamType.Currency;
                actions.Add(gainCurrency);
            }
        }
