...
namespace moonNest
{
    ...
    public class UserInventory : RemotableUserData<FirestoreUserData>
    {
        ...
        public static ChestKey FindOrCreateChestKey(int chestId) => Ins.FindOrCreateChestKey_Internal(chestId);
        #endregion
        ...
        {
            if (stats.Length == 0) return new List<T>() { FindById<T>(detailId) as T };
        ...
        #region chest key
        internal ChestKey FindChestKey_Internal(int chestId)
            => chestKeys.TryGetValue(chestId, out var chestKey) ? chestKey : null;

        internal ChestKey FindOrCreateChestKey_Internal(int chestId)
            => chestKeys.GetOrCreate(chestId, id => new ChestKey(id));
