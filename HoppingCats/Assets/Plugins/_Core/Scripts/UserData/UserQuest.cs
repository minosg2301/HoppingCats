...
namespace moonNest
{
    ...
    public class UserQuest : RemotableUserDataGroup<FirestoreUserData, Quest, QuestDetail, QuestGroup, QuestGroupDetail>
    {
        ...
        protected override Quest CreateNew(QuestDetail detail)
        {
            ...
            if (requireStatId != -1) RegisterStatHandler(quest, requireStatId);
            return quest;
        }
        ...
        protected internal override void OnLoad()
        {
            ...
            foreach (var quest in datas.Values)
            {
                var requireStatId = quest.Detail.statRequire.statId;
                if (requireStatId != -1) RegisterStatHandler(quest, requireStatId);
            }
        ...
        internal static void DoAction(ActionData action, long value = 1, List<Quest> listQuest = null)
        {
        ...
        public static void DoClaim(Quest quest, int rewardMultiply = 1)
        {
            if (quest.CanClaim && !quest.Claimed)
            {
                ...
                if (quest.Detail.removeOnClaimed)
                {
                    ...
                    // unregister stat handler
                    var requireStatId = quest.Detail.statRequire.statId;
                    if (requireStatId != -1)
                        userQuest.UnregisterStatHandler(quest, requireStatId);
                }
        ...
        public async void ActiveNewGroups(List<int> activeOnCompleteds)
        {
            ...
            foreach (int groupId in activeOnCompleteds)
            {
                ...
                {
                    if (quest.ActionRequire.action.id == ActionDefinition.kLoginDay)
        ...
        readonly Dictionary<int, Action<BaseUserData>> statHandlers = new();
        ...
        void RegisterStatHandler(Quest quest, int requireStatId)
        {
            if (quest && !statHandlers.ContainsKey(quest.Id))
            {
                statHandlers[quest.Id] = (data) => UpdateQuestByStat(quest, requireStatId);
                UserData.Ins.Subscribe(requireStatId.ToString(), statHandlers[quest.Id]);
            }
        }

        void UnregisterStatHandler(Quest quest, int requireStatId)
        {
            if (quest && statHandlers.ContainsKey(quest.Id))
            {
                UserData.Ins.Unsubscribe(requireStatId.ToString(), statHandlers[quest.Id]);
                statHandlers.Remove(quest.Id);
            }
        }
