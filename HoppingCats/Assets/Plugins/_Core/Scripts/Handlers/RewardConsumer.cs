...
namespace moonNest
{
    ...
    public class RewardConsumer
    {
        ...
        static bool currencyChanged = false;
        static bool itemChanged = false;
        static bool statChanged = false;
        static bool chestKeyChanged = false;
        ...
        public static ConsumedRewards ConsumeIAPPackage(InStorePackage inStorePackage, int multiply = 1)
        {
            ...
            if (itemChanged || chestKeyChanged) UserInventory.Ins.Save();
            if (statChanged) UserData.Ins.Save();
        ...
        {
            rewardResult.Clear();
            currencyChanged = itemChanged = statChanged = chestKeyChanged = false;
        }
        ...
        private static void Apply(ChestReward chestReward, int multiply = 1)
        {
            ...
            {
                ChestKey chestKey = UserInventory.FindOrCreateChestKey(chest.id);
