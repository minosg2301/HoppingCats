...
namespace moonNest
{
    public class UIReward : BaseUIData<RewardObject>
    {
        ...
        public override void SetData(RewardObject reward)
        {
            ...
            if (nameText)
            {
                if (!nameTextForStatOnly || (nameTextForStatOnly && reward.type == RewardType.Stat))
                {
                    ...
                    nameText.text = reward.DisplayName.ToLocalized();
                }
                else
                {
            ...
            if (infoButton)
            {
                switch (reward.type)
                {
                    ...
                    case RewardType.Item:
                        infoButton.Interactable = Tooltip.requestItemTooltip != null
                            || Tooltip.requestRandomItemTooltip != null; break;
        ...
        public virtual void SetData(ItemReward itemReward)
        {
            ...
            if (nameText) nameText.text = itemReward.DisplayName.ToLocalized();
            if (amountText)
            {
