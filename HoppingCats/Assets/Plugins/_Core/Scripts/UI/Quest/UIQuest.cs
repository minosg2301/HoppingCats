...
namespace moonNest
{
    public class UIQuest : BaseUIData<Quest>, IObserver
    {
        ...
        protected virtual void UpdateUI()
        {
            ...
            var progressValue = Quest.progress.value;
            var require = Quest.IsUseMultiRequires ? Quest.ActionRequires.count : (Quest.StatRequire.statId != -1 ? Quest.StatRequire.value : Quest.ActionRequire.count);
            if (progressor)
            {
                progressor.SetMin(0);
