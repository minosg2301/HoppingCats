...
class TutorialDetailTabDrawer : ListTabDrawer<TutorialDetail>
{
    ...
    protected override void DoDrawContent(TutorialDetail element)
    {
        ...
        Draw.Space(60);
        Draw.BeginVertical();
        element.nonForce = Draw.ToggleField("Non Force", element.nonForce, 80);
        element.allowSkip = Draw.ToggleField("Allow Skip", element.allowSkip, 80);
        if (element.allowSkip)
            element.resetOnSkip = Draw.ToggleField("Reset On Skip", element.resetOnSkip, 80);
