...
public class QuestDetailTab : ListTabDrawer<QuestDetail>
{
    ...
    private void DoDrawRequire(StatRequireDetail require)
    {
        ...
        Draw.FlexibleSpace();
        Draw.EndHorizontal();
        require.value = Draw.LongField("Value", require.value, 120);
    }
    ...
    void DrawActionParam(ActionParam param, float width)
    {
        switch (param.type)
        {
            ...
            case ActionParamType.Tutorial: param.content = Draw.IntPopupField("Tutorial Id", param.content, TutorialAsset.Ins.tutorials, "name", "id", width); break;
        }
    }
